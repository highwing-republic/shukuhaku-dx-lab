...
DATA_FILE = Path(__file__).with_name("occupancy_history.csv")
...
def load_history() -> pd.DataFrame:
    ...
    if not DATA_FILE.exists():
        raise FileNotFoundError(f"{DATA_FILE.name} がapp.pyと同じフォルダにありません。")
...
st.subheader("宿泊DXラボ ダッシュボード")
nav_col1, nav_col2, nav_col3 = st.columns(3)

with nav_col1:
    with st.container(border=True):
        st.markdown("### 📊 人手不足シミュレーション")
...
with nav_col3:
    with st.container(border=True):
        st.markdown("### 📈 客室稼働率の長期推移")
        st.write("都道府県・施設タイプ別の客室稼働率を比較できます。")
        st.page_link(
            "app.py",
            label="長期推移を表示",
            icon="📈",
...
try:
    ...
    st.code(str(exc))
    st.info("app.pyとoccupancy_history.csvを同じフォルダに置いてください。")
    st.stop()
